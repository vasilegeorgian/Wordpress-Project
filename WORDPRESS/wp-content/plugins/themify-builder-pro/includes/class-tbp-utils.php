...
class Tbp_Utils {
   ...
   public static  function localize_predesigned_templates( $data ) {
	...
	public static function module_categories( $categories ) {
		...
		$categories = array_merge( $categories, array(
			'archive' => array( 'label' => __( 'Archive', 'themify' ), 'active' => false ),
			'single' => array( 'label' => __( 'Single', 'themify' ), 'active' => false ),
			'product_single' => array( 'label' => __( 'Product Single', 'themify' ), 'active' => false ),
			'product_archive' => array( 'label' => __( 'Product Archive', 'themify' ), 'active' => false ),
		) );
	...
	*/
	 ...
	 * @return array
	 */
	function get_ptb_fields_by_type( $type ) {
		$options = array();
		if ( ! class_exists( 'PTB' ) ) {
			return $options;
		}
