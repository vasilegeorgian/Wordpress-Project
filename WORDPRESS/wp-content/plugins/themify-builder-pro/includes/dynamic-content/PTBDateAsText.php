...
class Tbp_Dynamic_Item_PTBDateAsText extends Tbp_Dynamic_Item {
	...
	function get_label() {
		return __( 'PTB Custom Fields (Date)', 'themify' );
	}
	...
	function get_options() {
		...
		return array(
			...
			array(
				...
				'type' => 'select',
				'options' => array(
					'start_date' => __( 'Start Date', 'themify' ),
					'end_date' => __( 'End Date', 'themify' ),
				),
			),
			array(
			...
			array(
				'label' => __( 'Custom Date Format', 'themify' ),
				'id' => 'custom_date_format',
				'type' => 'text',
				'help' => sprintf( __( 'For information on how to format date and time see <a href="%s" target="_blank">Codex</a>.', 'themify' ), 'https://codex.wordpress.org/Formatting_Date_and_Time' )
			),
		);
	}
}