...
class ParserState
{
     ...
     * @param int $iLineNo
     */
    public function __construct($sText, \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Settings $oParserSettings, $iLineNo = 1)
    {
    ...
    public function parseIdentifier($bIgnoreCase = \true)
    {
        ...
        if ($sResult === null) {
            throw new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\UnexpectedTokenException($sResult, $this->peek(5), 'identifier', $this->iLineNo);
        }
        $sCharacter = null;
    ...
    public function consume($mValue = 1)
    {
        if (\is_string($mValue)) {
            ...
            if (!$this->streql($this->substr($this->iCurrentPosition, $iLength), $mValue)) {
                throw new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\UnexpectedTokenException($mValue, $this->peek(\max($iLength, 5)), $this->iLineNo);
            }
            ...
            if ($this->iCurrentPosition + $mValue > $this->iLength) {
                throw new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\UnexpectedEOFException($mValue, $this->peek(5), 'count', $this->iLineNo);
            }
    ...
    public function consumeExpression($mExpression, $iMaxLength = null)
    {
        ...
        throw new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\UnexpectedTokenException($mExpression, $this->peek(5), 'expression', $this->iLineNo);
    }
    /**
    ...
    public function consumeUntil($aEnd, $bIncludeEnd = \false, $consumeEnd = \false, array &$comments = [])
    {
        ...
        throw new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\UnexpectedEOFException('One of ("' . \implode('","', $aEnd) . '")', $this->peek(5), 'search', $this->iLineNo);
    }
    /**
     * @return string
     */
