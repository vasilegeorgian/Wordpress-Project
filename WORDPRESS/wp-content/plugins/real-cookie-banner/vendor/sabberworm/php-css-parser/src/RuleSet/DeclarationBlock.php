...
class DeclarationBlock extends \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\RuleSet\RuleSet
{
     ...
     */
    public static function parse(\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\ParserState $oParserState, $oList = null)
    {
        $aComments = [];
        $oResult = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\RuleSet\DeclarationBlock($oParserState->currentLine());
        try {
            $aSelectorParts = [];
    ...
    public function setSelectors($mSelector, $oList = null)
    {
        ...
        foreach ($this->aSelectors as $iKey => $mSelector) {
            if (!$mSelector instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\Selector) {
                if ($oList === null || !$oList instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\CSSList\KeyFrame) {
                    if (!\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\Selector::isValid($mSelector)) {
                        throw new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\UnexpectedTokenException("Selector did not match '" . \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\Selector::SELECTOR_VALIDATION_RX . "'.", $mSelector, "custom");
                    }
                    $this->aSelectors[$iKey] = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\Selector($mSelector);
                } else {
                    if (!\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\KeyframeSelector::isValid($mSelector)) {
                        throw new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\UnexpectedTokenException("Selector did not match '" . \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\KeyframeSelector::SELECTOR_VALIDATION_RX . "'.", $mSelector, "custom");
                    }
                    $this->aSelectors[$iKey] = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Property\KeyframeSelector($mSelector);
                }
            }
        }
    }
    ...
    public function expandBorderShorthand()
    {
        ...
        foreach ($aBorderRules as $sBorderRule) {
            ...
            foreach ($aValues as $mValue) {
                ...
                $oNewRule = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Rule\Rule($sNewRuleName, $oRule->getLineNo(), $oRule->getColNo());
    ...
    public function expandDimensionsShorthand()
    {
        ...
        foreach ($aExpansions as $sProperty => $sExpanded) {
            ...
            foreach (['top', 'right', 'bottom', 'left'] as $sPosition) {
                $oNewRule = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Rule\Rule(\sprintf($sExpanded, $sPosition), $oRule->getLineNo(), $oRule->getColNo());
                $oNewRule->setIsImportant($oRule->getIsImportant());
    ...
    public function expandFontShorthand()
    {
        ...
        foreach ($aValues as $mValue) {
            ...
            } elseif (\in_array($mValue, ['bold', 'bolder', 'lighter']) || $mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size && \in_array($mValue->getSize(), \range(100, 900, 100))) {
                $aFontProperties['font-weight'] = $mValue;
            } elseif ($mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList && $mValue->getListSeparator() == '/') {
                list($oSize, $oHeight) = $mValue->getListComponents();
                $aFontProperties['font-size'] = $oSize;
                $aFontProperties['line-height'] = $oHeight;
            } elseif ($mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size && $mValue->getUnit() !== null) {
        ...
        foreach ($aFontProperties as $sProperty => $mValue) {
            $oNewRule = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Rule\Rule($sProperty, $oRule->getLineNo(), $oRule->getColNo());
    ...
    public function expandBackgroundShorthand()
    {
        ...
        $aBgProperties = ['background-color' => ['transparent'], 'background-image' => ['none'], 'background-repeat' => ['repeat'], 'background-attachment' => ['scroll'], 'background-position' => [new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size(0, '%', null, \false, $this->iLineNo), new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size(0, '%', null, \false, $this->iLineNo)]];
        $mRuleValue = $oRule->getValue();
        $aValues = [];
        if (!$mRuleValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList) {
            $aValues[] = $mRuleValue;
        ...
        foreach ($aValues as $mValue) {
            ...
            } elseif (\in_array($mValue, ['left', 'center', 'right', 'top', 'bottom']) || $mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Size) {
                if ($iNumBgPos == 0) {
    ...
    public function createShorthandProperties(array $aProperties, $sShorthand)
    {
        ...
        if (\count($aNewValues)) {
            $oNewRule = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Rule\Rule($sShorthand, $oRule->getLineNo(), $oRule->getColNo());
    ...
    public function createFontShorthand()
    {
        ...
        $oNewRule = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Rule\Rule('font', $oOldRule->getLineNo(), $oOldRule->getColNo());
        unset($oOldRule);
    ...
    public function render(\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\OutputFormat $oOutputFormat)
    {
        ...
        if (\count($this->aSelectors) === 0) {
            ...
            throw new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\OutputException("Attempt to print declaration block with missing selector", $this->iLineNo);
        }
