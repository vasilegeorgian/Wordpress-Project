 ...
 */
class Rule implements \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Renderable, \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Comment\Commentable
{
    /**
     * @var string
     */
    ...
    public static function parse(\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\ParserState $oParserState)
    {
        ...
        $oRule = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Rule\Rule($oParserState->parseIdentifier(!$oParserState->comes("--")), $oParserState->currentLine(), $oParserState->currentColumn());
        $oRule->setComments($aComments);
        $oRule->addComments($oParserState->consumeWhiteSpace());
        $oParserState->consume(':');
        $oValue = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Value::parseValue($oParserState, self::listDelimiterForRule($oRule->getRule()));
    ...
    public function setValues(array $aSpaceSeparatedValues)
    {
        ...
        if (\count($aSpaceSeparatedValues) > 1) {
            $oSpaceSeparatedList = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList(' ', $this->iLineNo);
        }
        foreach ($aSpaceSeparatedValues as $aCommaSeparatedValues) {
            $oCommaSeparatedList = null;
            if (\count($aCommaSeparatedValues) > 1) {
                $oCommaSeparatedList = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList(',', $this->iLineNo);
            }
    ...
    public function getValues()
    {
        ...
        foreach ($this->mValue->getListComponents() as $mValue) {
            if (!$mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList || $mValue->getListSeparator() !== ',') {
                $aResult[] = [$mValue];
                continue;
            }
    ...
    public function addValue($mValue, $sType = ' ')
    {
        ...
        if (!$this->mValue instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList || $this->mValue->getListSeparator() !== $sType) {
            $mCurrentValue = $this->mValue;
            $this->mValue = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList($sType, $this->iLineNo);
            if ($mCurrentValue) {
