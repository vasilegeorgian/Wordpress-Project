...
abstract class Value implements \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Renderable
{
     ...
     */
    public static function parseValue(\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\ParserState $oParserState, array $aListDelimiters = [])
    {
        ...
        foreach ($aListDelimiters as $sDelimiter) {
            ...
            while (($iStartPosition = \array_search($sDelimiter, $aStack, \true)) !== \false) {
                ...
                $oList = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\RuleValueList($sDelimiter, $oParserState->currentLine());
     ...
     * @throws UnexpectedTokenException
     */
    public static function parseIdentifierOrFunction(\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\ParserState $oParserState, $bIgnoreCase = \false)
    {
        ...
        if ($oParserState->comes('(')) {
            ...
            $sResult = new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\CSSFunction($sResult, $aArguments, ',', $oParserState->currentLine());
            $oParserState->consume(')');
        }
        return $sResult;
    }
     ...
     */
    public static function parsePrimitiveValue(\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\ParserState $oParserState)
    {
        $oValue = null;
        $oParserState->consumeWhiteSpace();
     ...
     * @throws UnexpectedTokenException
     */
    private static function parseMicrosoftFilter(\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\ParserState $oParserState)
    {
        $sFunction = $oParserState->consumeUntil('(', \false, \true);
        $aArguments = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\Value::parseValue($oParserState, [',', '=']);
        return new \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Value\CSSFunction($sFunction, $aArguments, ',', $oParserState->currentLine());
    }
    /**
     * @return string
     *
     ...
     * @throws UnexpectedTokenException
     */
    private static function parseUnicodeRangeValue(\DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\Parsing\ParserState $oParserState)
    {
