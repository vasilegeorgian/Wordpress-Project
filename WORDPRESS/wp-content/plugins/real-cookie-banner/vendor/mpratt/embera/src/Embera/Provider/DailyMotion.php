 ...
 */
class DailyMotion extends \DevOwl\RealCookieBanner\Vendor\Embera\Provider\ProviderAdapter implements \DevOwl\RealCookieBanner\Vendor\Embera\Provider\ProviderInterface
{
    /** inline {@inheritdoc} */
    ...
    /** inline {@inheritdoc} */
    protected static $hosts = ['*.dailymotion.com', 'dai.ly'];
    /** inline {@inheritdoc} */
    ...
    public function normalizeUrl(\DevOwl\RealCookieBanner\Vendor\Embera\Url $url)
    {
        ...
        if (\preg_match('~dailymotion\\.com/(?:embed/)?(?:video|live)/([^/]+)/?~i', (string) $url, $matches)) {
            ...
            if (\preg_match('~dai\\.ly/([^/]+)/?~i', (string) $url, $matches)) {
                $url->overwrite('https://www.dailymotion.com/video/' . $matches['1']);
            }
        }
        return $url;
