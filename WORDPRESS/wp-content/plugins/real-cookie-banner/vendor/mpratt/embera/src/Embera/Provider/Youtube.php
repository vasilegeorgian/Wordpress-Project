 ...
 */
class Youtube extends \DevOwl\RealCookieBanner\Vendor\Embera\Provider\ProviderAdapter implements \DevOwl\RealCookieBanner\Vendor\Embera\Provider\ProviderInterface
{
    /** inline {@inheritdoc} */
    protected $endpoint = 'https://www.youtube.com/oembed?format=json&scheme=https';
    /** inline {@inheritdoc} */
    protected static $hosts = ['m.youtube.com', 'youtube.com', 'youtu.be'];
    /** inline {@inheritdoc} */
    ...
    public function getFakeResponse()
    {
        if (\preg_match('~v=([a-z0-9_\\-]+)~i', (string) $this->url, $matches) || \preg_match('~/shorts/([^/]+)~i', (string) $this->url, $matches)) {
            ...
            if (\preg_match('~/playlist(.+)list=([a-z0-9_\\-]+)~i', (string) $this->url, $matches)) {
