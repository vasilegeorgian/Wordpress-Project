 ...
 */
namespace DevOwl\RealCookieBanner\Vendor\Embera;

use DevOwl\RealCookieBanner\Vendor\Embera\Http\HttpClient;
use DevOwl\RealCookieBanner\Vendor\Embera\Http\OembedClient;
use DevOwl\RealCookieBanner\Vendor\Embera\Http\HttpClientInterface;
use DevOwl\RealCookieBanner\Vendor\Embera\Html\IgnoreTags;
use DevOwl\RealCookieBanner\Vendor\Embera\Html\ResponsiveEmbeds;
use DevOwl\RealCookieBanner\Vendor\Embera\ProviderCollection\ProviderCollectionInterface;
use DevOwl\RealCookieBanner\Vendor\Embera\ProviderCollection\DefaultProviderCollection;
/**
 * Embera
...
class Embera
{
     ...
     * @return void
     */
    public function __construct(array $config = [], \DevOwl\RealCookieBanner\Vendor\Embera\ProviderCollection\ProviderCollectionInterface $collection = null, \DevOwl\RealCookieBanner\Vendor\Embera\Http\HttpClientInterface $httpClient = null)
    {
        $this->config = \array_merge(['https_only' => \false, 'fake_responses' => self::ALLOW_FAKE_RESPONSES, 'ignore_tags' => ['pre', 'code', 'a', 'img', 'iframe', 'oembed'], 'responsive' => \false, 'width' => 0, 'height' => 0, 'maxheight' => 0, 'maxwidth' => 0, 'referer' => '', 'curl_params' => [], 'file_get_contents_params' => []], $config);
        ...
        if (!$collection) {
            $this->providerCollection = new \DevOwl\RealCookieBanner\Vendor\Embera\ProviderCollection\DefaultProviderCollection($this->config);
        }
        $this->httpClient = $httpClient;
        if (!$httpClient) {
            $this->httpClient = new \DevOwl\RealCookieBanner\Vendor\Embera\Http\HttpClient($this->config);
        }
        ...
        if (\is_string($text)) {
            ...
            if (!empty($table)) {
                if (!empty($this->config['ignore_tags']) && \strpos($text, '>') !== \false) {
                    $ignoreTags = new \DevOwl\RealCookieBanner\Vendor\Embera\Html\IgnoreTags($this->config['ignore_tags']);
    ...
    public function getUrlData($urls)
    {
        ...
        foreach ($this->providerCollection->findProviders($urls) as $url => $provider) {
            try {
                $oembedClient = new \DevOwl\RealCookieBanner\Vendor\Embera\Http\OembedClient($this->config, $this->httpClient);
                $response = $oembedClient->getResponseFrom($provider);
                if ($this->config['responsive'] && !$provider->hasResponsiveSupport()) {
                    $responsive = new \DevOwl\RealCookieBanner\Vendor\Embera\Html\ResponsiveEmbeds();
