 ...
 */
class HttpClient implements \DevOwl\RealCookieBanner\Vendor\Embera\Http\HttpClientInterface
{
    ...
    {
        $this->config = \array_merge(['use_curl' => \true, 'user_agent' => 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.212 Safari/537.36', 'referer' => '', 'curl_params' => [], 'file_get_contents_params' => []], $config);
    }
    /** inline {@inheritdoc} */
    ...
    protected function fetchWithCurl($url, array $params = [])
    {
        ...
        $options = $params + array(\CURLOPT_USERAGENT => $this->config['user_agent'], \CURLOPT_ENCODING => '', \CURLOPT_FOLLOWLOCATION => \true, \CURLOPT_SSL_VERIFYHOST => 0, \CURLOPT_SSL_VERIFYPEER => 0, \CURLINFO_HEADER_OUT => \true);
    ...
    protected function fetchWithFileGetContents($url, array $params = [])
    {
        ...
        $params = \array_merge(['method' => 'GET', 'user_agent' => $this->config['user_agent'], 'follow_location' => 1, 'max_redirects' => 20, 'timeout' => 40], $params);
        if (!empty($this->config['referer'])) {
