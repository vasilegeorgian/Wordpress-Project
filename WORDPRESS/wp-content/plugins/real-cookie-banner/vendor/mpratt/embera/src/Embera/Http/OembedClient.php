 ...
 */
namespace DevOwl\RealCookieBanner\Vendor\Embera\Http;

use DevOwl\RealCookieBanner\Vendor\Embera\Embera;
use DevOwl\RealCookieBanner\Vendor\Embera\Http;
use DevOwl\RealCookieBanner\Vendor\Embera\Provider\ProviderInterface;
/**
...
class OembedClient
{
     ...
     * @return void
     */
    public function __construct($config, \DevOwl\RealCookieBanner\Vendor\Embera\Http\HttpClientInterface $http)
    {
        $this->config = $config;
        $this->http = $http;
    }
     ...
     * @return array
     */
    public function getResponseFrom(\DevOwl\RealCookieBanner\Vendor\Embera\Provider\ProviderInterface $provider)
    {
        if ($this->config['fake_responses'] == \DevOwl\RealCookieBanner\Vendor\Embera\Embera::ONLY_FAKE_RESPONSES) {
     ...
     */
    protected function lookup(\DevOwl\RealCookieBanner\Vendor\Embera\Provider\ProviderInterface $provider)
    {
        $url = $this->constructUrl($provider->getEndpoint(), $provider->getParams());
        $response = $this->http->fetch($url);
        $json = \json_decode($response, \true);
        if ($json) {
            return \array_merge($json, ['embera_using_fake_response' => 0, 'embera_provider_name' => $provider->getProviderName()]);
        }
        if ($this->config['fake_responses'] == \DevOwl\RealCookieBanner\Vendor\Embera\Embera::ALLOW_FAKE_RESPONSES) {
    ...
    {
        $defaultValues = ['version' => '1.0', 'provider_name' => '', 'url' => '', 'title' => '', 'author_name' => '', 'author_url' => '', 'cache_age' => 0, 'embera_using_fake_response' => 1, 'embera_provider_name' => $providerName];
