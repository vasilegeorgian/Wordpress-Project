 ...
 *
 */
class Zingsoft extends \DevOwl\RealCookieBanner\Vendor\Embera\Provider\ProviderAdapter implements \DevOwl\RealCookieBanner\Vendor\Embera\Provider\ProviderInterface
{
    /** inline {@inheritdoc} */
    protected $endpoint = 'https://app.zingsoft.com/oembed?format=json';
    /** inline {@inheritdoc} */
    protected static $hosts = ['*.zingsoft.com'];
    /** inline {@inheritdoc} */
    ...
    public function modifyResponse(array $response = [])
    {
        if (!empty($response['html'])) {
            $response['html'] = \trim(\preg_replace('~\\s+~', ' ', \preg_replace('~\\n~i', ' ', $response['html'])));
        }
        return $response;
    }
    /** inline {@inheritdoc} */
    public function getFakeResponse()
    {
        ...
        $attr[] = 'height="{height}"';
        $attr[] = 'src="' . $this->url . '"';
        return ['type' => 'rich', 'provider_name' => 'Zingsoft', 'provider_url' => 'https://*.zingsoft', 'title' => 'Unknown title', 'html' => '<iframe ' . \implode(' ', $attr) . '></iframe>'];
    }
}
