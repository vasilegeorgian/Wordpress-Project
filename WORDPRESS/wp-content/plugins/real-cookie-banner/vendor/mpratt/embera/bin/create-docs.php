 ...
 */
namespace DevOwl\RealCookieBanner\Vendor\Embera\bin;

use ReflectionClass;
use RuntimeException;
use DevOwl\RealCookieBanner\Vendor\Embera\ProviderCollection\DefaultProviderCollection;
use DevOwl\RealCookieBanner\Vendor\Embera\ProviderCollection\SlimProviderCollection;
...
$defaultProviderCollectionProviders = getProvidersFromCollection(new \DevOwl\RealCookieBanner\Vendor\Embera\ProviderCollection\DefaultProviderCollection());
$slimProviderCollectionProviders = getProvidersFromCollection(new \DevOwl\RealCookieBanner\Vendor\Embera\ProviderCollection\SlimProviderCollection());
$processedProviders = [];
foreach ($defaultProviderCollectionProviders as $host => $provider) {
    ...
    $providerDescription = \trim(\str_replace(['*', '/'], "\n", $providerDoc));
    $providerDescription = \trim(\preg_replace('~\\s+~', ' ', \preg_replace('~(.+)@link.+~is', '$1', $providerDescription)));
    $providerData = ['{provider_name}' => $providerName, '{provider_description}' => \wordwrap($providerDescription), '{provider_url}' => $providerLink, '{provider_doc_url}' => $providerLinkDocumentation, '{provider_https}' => $providerHttps, '{provider_fake}' => $providerFake, '{provider_responsive}' => $providerResponsive, '{provider_collections}' => \implode(' , ', $providerCollections), '{provider_params}' => \implode(' , ', $providerParams), '{provider_hosts}' => \implode(' , ', $providerHosts), '{provider_todo}' => $providerTodo];
    ...
    $processedProviders[$providerName] = \true;
}
