...
class Decoder
{
    ...
    public function decode(int $offset) : array
    {
        ...
        if ($type === self::_EXTENDED) {
            $nextByte = \ord(\DevOwl\RealCookieBanner\Vendor\MaxMind\Db\Reader\Util::read($this->fileStream, $offset, 1));
            $type = $nextByte + 7;
            if ($type < 8) {
                throw new \DevOwl\RealCookieBanner\Vendor\MaxMind\Db\Reader\InvalidDatabaseException('Something went horribly wrong in the decoder. An extended type ' . 'resolved to a type number < 8 (' . $type . ')');
            }
            ++$offset;
        }
    ...
    private function decodeByType(int $type, int $offset, int $size) : array
    {
        ...
        switch ($type) {
            ...
            default:
                throw new \DevOwl\RealCookieBanner\Vendor\MaxMind\Db\Reader\InvalidDatabaseException('Unknown or unexpected type: ' . $type);
        }
    }
    private function verifySize(int $expected, int $actual) : void
    {
        if ($expected !== $actual) {
            throw new \DevOwl\RealCookieBanner\Vendor\MaxMind\Db\Reader\InvalidDatabaseException("The MaxMind DB file's data section contains bad data (unknown data type or corrupt data)");
        }
    }
    ...
    private function decodePointer(int $ctrlByte, int $offset) : array
    {
        ...
        $buffer = \DevOwl\RealCookieBanner\Vendor\MaxMind\Db\Reader\Util::read($this->fileStream, $offset, $pointerSize);
        ...
        switch ($pointerSize) {
            ...
            case 4:
                ...
                break;
            default:
                throw new \DevOwl\RealCookieBanner\Vendor\MaxMind\Db\Reader\InvalidDatabaseException('Unexpected pointer size ' . $pointerSize);
        }
    ...
    private function decodeUint(string $bytes, int $byteLength)
    {
        ...
        $useRealInts = $byteLength <= _MM_MAX_INT_BYTES || $byteLength === _MM_MAX_INT_BYTES + 1 && (\ord($bytes[0]) & 0x80) === 0;
