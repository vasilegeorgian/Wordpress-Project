...
class AutoptimizeCacheImpl extends \DevOwl\RealCookieBanner\Vendor\DevOwl\CacheInvalidate\AbstractCache {
    ...
    public function excludeAssetsHook($excludeAssets) {
        foreach (['js', 'css'] as $type) {
            add_filter(\sprintf('autoptimize_filter_%s_exclude', $type), function ($excluded) use (
                $excludeAssets,
                $type
            ) {
            ...
            add_filter(\sprintf('autoptimize_filter_%s_consider_minified', $type), function ($considerMinified) use (
                $excludeAssets,
                $type
            ) {
