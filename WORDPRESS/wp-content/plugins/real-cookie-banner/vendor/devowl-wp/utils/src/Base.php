...
trait Base {
    ...
    public function debug($message, $methodOrFunction = null) {
        if ($this->getPluginConstant(\DevOwl\RealCookieBanner\Vendor\MatthiasWeb\Utils\Constants::PLUGIN_CONST_DEBUG)) {
            $log =
    ...
    public function getTableName($name = '') {
        ...
        return $wpdb->prefix .
            $this->getPluginConstant(
                \DevOwl\RealCookieBanner\Vendor\MatthiasWeb\Utils\Constants::PLUGIN_CONST_DB_PREFIX
            ) .
            (empty($name) ? '' : '_' . $name);
    }
}
