    ...
    public function localizeScript($context) {
        ...
        $restQuery = apply_filters('DevOwl/Utils/RestQuery', [
            '_v' => $this->getPluginConstant(
                \DevOwl\RealCookieBanner\Vendor\MatthiasWeb\Utils\Constants::PLUGIN_CONST_VERSION
            ),
...
',
    ...
    public function getChunkTranslationMap() {
        $inc = $this->getPluginConstant(\DevOwl\RealCookieBanner\Vendor\MatthiasWeb\Utils\Constants::PLUGIN_CONST_INC);
    ...
    protected function getTranslationSuffixByBasename($basename) {
        ...
        $path = $this->getPluginConstant(
            \DevOwl\RealCookieBanner\Vendor\MatthiasWeb\Utils\Constants::PLUGIN_CONST_PATH
        );
    ...
    public function enqueueMobx() {
        ...
        $this->enqueueLibraryScript(\DevOwl\RealCookieBanner\Vendor\MatthiasWeb\Utils\Constants::ASSETS_HANDLE_MOBX, [
    ...
    protected function enqueue(
        ...
        $useHandle = $isLib
            ? $handle
            : $this->getPluginConstant(\DevOwl\RealCookieBanner\Vendor\MatthiasWeb\Utils\Constants::PLUGIN_CONST_SLUG) .
                '-' .
                $handle;
        if (!\is_array($src)) {
            $src = [$src];
    ...
    protected function enqueueComposer(
        ...
        if (\file_exists($composerPath)) {
            ...
            $url = plugins_url(
                $packageSrc,
                $this->getPluginConstant(\DevOwl\RealCookieBanner\Vendor\MatthiasWeb\Utils\Constants::PLUGIN_CONST_FILE)
            );
...
JS;
    ...
    public function getCachebusterVersion($src, $isLib = \false, $default = null) {
        ...
        $path =
            $this->getPluginConstant(\DevOwl\RealCookieBanner\Vendor\MatthiasWeb\Utils\Constants::PLUGIN_CONST_INC) .
            '/base/others/';
...
<script %7$s id="a%1$s2-js-extra">var %3$s = /* document.write */ JSON.parse(window.atob(document.getElementById("a%1$s1-js-extra").innerHTML.substr(%5$d)));</script>',
                            $uuid,
