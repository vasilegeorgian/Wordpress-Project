 ...
 */
class BlockableScanner extends \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\AbstractPlugin {
    ...
    public function blockedMatch($result, $matcher, $match) {
        ...
        // Never show double-scanned elements when they e.g. caught by two rules and a rerun through `Utils::preg_replace_callback_recursive`
        $markup = $result->getMarkup();
        if (
            $markup instanceof \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Markup &&
            \strpos(
                $markup->getContent(),
                \sprintf(
                    '%s="%s"',
        ...
        if (
            ...
            $matcher instanceof
                \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\matcher\StyleInlineMatcher ||
            $matcher instanceof
                \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\matcher\StyleInlineAttributeMatcher
        ) {
            /**
             * Var.
             *
    ...
    protected function processBlockedByTagAttributeMatch($isBlocked, $linkAttribute, $link) {
        ...
        if (
            !$this->probablyMemorizeIsBlocked($isBlocked, $link, $tag, $linkAttribute) &&
            !$this->isStyleAttributeFalsePositive($linkAttribute, $link)
        ) {
    ...
    protected function processBlockedBySelectorSyntax($isBlocked, $m) {
        ...
        $this->probablyMemorizeIsBlocked(
            $isBlocked,
            $this->isNotAnExcludedUrl($possibleUrl) ? $possibleUrl : null,
            $m->getTag(),
            $m->getLinkAttribute()
        );
        return $isBlocked;
    ...
    protected function probablyMemorizeExternalUrl($isBlocked, $url, $tag, $attribute, $markup) {
        if (!$isBlocked->isBlocked() && !\in_array($tag, ['a'], \true) && $this->isNotAnExcludedUrl($url)) {
            $this->results[] = $entry = new \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\plugins\scanner\ScanEntry();
    ...
    public function filterFalsePositives() {
        $falsePositivesProcess = new \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\plugins\scanner\FalsePositivesProcessor(
            $this,
            $this->results
        );
