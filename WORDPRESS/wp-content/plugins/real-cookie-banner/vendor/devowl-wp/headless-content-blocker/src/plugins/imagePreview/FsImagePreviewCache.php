 ...
 */
abstract class FsImagePreviewCache extends
    \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\plugins\imagePreview\ImagePreviewCache {
    ...
    public function get($thumbnails) {
        foreach ($thumbnails as $thumbnail) {
            ...
            if (\DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Utils::endsWith($filename, '.failed')) {
    ...
    public function set($thumbnails) {
        ...
        foreach ($handles as $embedUrl => $handle) {
            ...
            if (\DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Utils::endsWith($embedUrl, '-alt')) {
        ...
        if ($status === 200 && \is_file($file)) {
            ...
            if ($dimensions !== \false) {
                ...
                if ($extension !== \false) {
                    ...
                    $newFile = path_join(\dirname($file), $newFilename);
                    if (\rename($file, $newFile) && \filesize($newFile) < self::MAX_DOWNLOAD_SIZE) {
                        ...
                        \file_put_contents(
                            path_join(\dirname($file), \sprintf('%s-%s.json', $id, $md5File)),
                            \json_encode([
                                'width' => $width,
                                'height' => $height,
                                'title' => $thumbnail->getTitle(),
                                'forceRatio' => $thumbnail->getForceRatio()
                            ])
                        );
                        return \true;
                    }
    ...
    protected function getCachedFilesForUrl($hash) {
        ...
        $res =
            $res === \false
                ? []
                : \array_values(
                    \array_filter($res, function ($f) {
                        return !\DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Utils::endsWith(
                            $f,
                            '.json'
                        );
                    })
