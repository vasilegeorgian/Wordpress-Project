...
class CssHelper {
    ...
    public static function removeBlanksFromCSSList($oList) {
        ...
        foreach ($oList->getContents() as $oBlock) {
            ...
            } elseif ($oBlock instanceof \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\RuleSet\AtRuleSet) {
                if (empty($oBlock->getRules())) {
                    $oList->remove($oBlock);
                }
            }
    ...
    public static function getCompactOutputFormat() {
        ...
        if ($outputFormat === null) {
            $outputFormat = \DevOwl\RealCookieBanner\Vendor\Sabberworm\CSS\OutputFormat::createCompact();
        }
        return $outputFormat;
    }
    /**
     ...
     */
    public static function removeNonBlockedRulesFromDocument(
        $document,
