 ...
 */
class ScriptInlineJsonBlocker extends \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\AbstractPlugin {
    private $schemas = [];
    /**
     * See `AbstractPlugin`.
     *
    ...
    public function checkResult($result, $matcher, $match) {
        if (
            ...
            foreach ($this->schemas as $schema) {
                $modifiedScript = $this->replaceScriptJsonContent(
                    $match,
                    $schema[0],
                    $schema[1],
                    $modified,
    ...
    public function replaceScriptJsonContent($match, $strpos, $pattern, &$result = null, $appendToScript = '') {
        ...
        $modifiedScript = \DevOwl\RealCookieBanner\Vendor\DevOwl\FastHtmlTag\Utils::preg_jit_safe($pattern, function (
            $pattern
        ) use ($match, &$result, $appendToScript) {
            return \preg_replace_callback(
                $pattern,
                function ($m) use (&$result) {
                    $json = \DevOwl\RealCookieBanner\Vendor\DevOwl\FastHtmlTag\Utils::isJson($m[2]);
                    if ($json) {
