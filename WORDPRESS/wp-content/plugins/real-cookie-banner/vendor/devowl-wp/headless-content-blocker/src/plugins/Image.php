 ...
 */
class Image extends \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\AbstractPlugin {
    ...
    public function blockedMatch($result, $matcher, $match) {
        if (
            ...
            $match->getTag() === self::HTML_TAG_IMG &&
            $match->hasAttribute(self::HTML_ATTRIBUTE_SRCSET)
        ) {
            $newSrcAttribute = \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\AttributesHelper::transformAttribute(
