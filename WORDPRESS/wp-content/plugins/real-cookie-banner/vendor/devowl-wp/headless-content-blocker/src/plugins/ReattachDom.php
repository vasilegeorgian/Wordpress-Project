 ...
 */
class ReattachDom extends \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\AbstractPlugin {
    private $matcher = [];
    /**
     * Init.
     */
    ...
    public function blockedMatch($result, $matcher, $match) {
        if ($result->isBlocked()) {
            foreach ($this->matcher as $fn) {
                if ($fn($match, $matcher)) {
                    $match->setAttribute(
                        \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Constants::HTML_ATTRIBUTE_REATTACH_DOM,
                        '1'
                    );
                    break;
                }
