...
class SelectorSyntaxFinder extends \DevOwl\RealCookieBanner\Vendor\DevOwl\FastHtmlTag\finder\TagAttributeFinder {
     ...
     *
     * @see https://regex101.com/r/vlbn3Y/5
     */
     ...
     */
    const EXPRESSION_REGEXP_INNER_SINGLE_ATTRIBUTE = '\\[([A-Za-z_-]+)(?:(%s)"([^"]+)")?(?:\\s*:\\s*(.*))?]';
    /**
     * C'tor.
     *
    ...
    public function createMatch($m) {
        ...
        $match = new \DevOwl\RealCookieBanner\Vendor\DevOwl\FastHtmlTag\finder\match\SelectorSyntaxMatch(
            $this,
            $m[0],
            $tag,
            $attributes,
            ...
            if (
                ...
                foreach ($attributeMatches as $attributeMatch) {
                    ...
                    $attributeInstances[] = new \DevOwl\RealCookieBanner\Vendor\DevOwl\FastHtmlTag\finder\SelectorSyntaxAttribute(
                        ...
                        $attributeMatch[4] ?? null
                    );
                }
                $finder = new \DevOwl\RealCookieBanner\Vendor\DevOwl\FastHtmlTag\finder\SelectorSyntaxFinder(
                    $expression,
                    $tag,
                    $attributeInstances
                );
