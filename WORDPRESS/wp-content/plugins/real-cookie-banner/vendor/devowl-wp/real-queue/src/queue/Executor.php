...
class Executor {
    ...
    public function run($ids, $maxExecutionTimeMs, $omitClientData = \false) {
        ...
        for ($i = 0; $i < \count($jobs); $i++) {
            ...
            if ($result === \false) {
                $result = new \WP_Error(
                    'real_queue_execute',
                    __('Process failed due to an unexpected error.', REAL_QUEUE_TD),
                    ['status' => 500]
                );
            }
        ...
        return [
            'done' => $done,
            'failed' => $failed,
            'remaining' => $this->core->getQuery()->readRemaining(),
            'pauseToAvoidRecurringException' => $pauseToAvoidRecurringException
        ];
    }
    /**
    ...
    public function buildErrorMessages($remaining) {
        ...
        foreach ($remaining as $type => $remainArr) {
            if ($remainArr['remaining'] === 0 && $remainArr['failure'] > 0) {
                 ...
                 * @param {string} $type
                 * @param {int[]} $remaining
                 * @param {Job[]} $failedJobs A list of maximum 5 items of failed jobs
                 * @return {string}
                 * @hook DevOwl/RealQueue/Error/Description
                 */
                $description = apply_filters(
                    'DevOwl/RealQueue/Error/Description',
                    \sprintf(
                        // translators:
                ...
                $errors['list'][$type] = [
                    'type' => $type,
                    'label' => \DevOwl\RealCookieBanner\Vendor\DevOwl\RealQueue\queue\Job::label($type),
                    'actions' => \DevOwl\RealCookieBanner\Vendor\DevOwl\RealQueue\queue\Job::actions($type),
    ...
    protected function isValidJob($job) {
        return $job->worker === \DevOwl\RealCookieBanner\Vendor\DevOwl\RealQueue\queue\Job::WORKER_SERVER;
    }
}
