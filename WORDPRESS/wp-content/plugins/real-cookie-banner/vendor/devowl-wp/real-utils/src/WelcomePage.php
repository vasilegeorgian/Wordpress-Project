...
class WelcomePage {
    ...
    public function activated_plugin($plugin) {
        ...
        if (!$fromBulkAction && !$isWpCli && $plugin === plugin_basename($this->getInitiator()->getPluginFile())) {
            $redirectState = \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::get(
                $this->getInitiator(),
                \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE,
                \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE_NOT_REGISTERED
            );
            if (
                $redirectState ===
                \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE_NOT_REGISTERED
                ...
                \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::set(
                    $this->getInitiator(),
                    \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE,
                    \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE_AWAITING_REDIRECT
                );
            }
        }
    }
        ...
        if (
            ...
            if (
                ...
                \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::set(
                    $this->getInitiator(),
                    \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE,
                    \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_REDIRECT_AFTER_ACTIVATE_REDIRECTED
                );
    ...
    <div class="about-wrap-content">
        ...
        $heroButton = isset($heroButton)
            ? \sprintf(
                ...
                esc_url(add_query_arg('feature', 'welcome-page', $heroButton[1])),
                $heroButton[0]
            )
            : '';
        $data = [
