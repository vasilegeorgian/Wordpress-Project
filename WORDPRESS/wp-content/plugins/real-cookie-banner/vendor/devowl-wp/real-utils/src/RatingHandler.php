...
class RatingHandler {
    ...
    public function dismiss($slug, $force) {
        ...
        if (
            \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::set(
                $initiator,
                \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_NEXT_RATING,
                $ts
            )
        ) {
            return $ts;
    ...
    public function getCanBeRated() {
        ...
        foreach (\DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\Core::getInstance()->getInitiators() as $initiator) {
            $ts = \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::get(
                $initiator,
                \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_NEXT_RATING
            );
            // Initialize default
    ...
    public static function instance() {
        return new \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\RatingHandler();
    }
}
