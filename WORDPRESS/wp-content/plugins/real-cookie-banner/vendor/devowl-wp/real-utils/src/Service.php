...
use DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\cross\AbstractCrossSelling;
use WP_Error;
use WP_REST_Request;
...
class Service {
    ...
    protected function sendDeactivationFeedbackEmail($apiHost, $slug, $email, $reason, $note) {
        $initiator = \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\Core::getInstance()->getInitiator($slug);
        if ($initiator === null) {
...
' .
    ...
    public function routeCrossDismiss($request) {
        ...
        $impl = \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\Core::getInstance()->getCrossSelling($slug);
        if ($impl === null) {
            return new \WP_Error(
                'rest_error',
    ...
    public function routeRatingDismiss($request) {
        $slug = $request->get_param('slug');
        $force = $request->get_param('force');
        $result = \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\Core::getInstance()
            ->getRatingHandler()
            ->dismiss($slug, $force);
        // Currently, ignore errors
        return new \WP_REST_Response(['result' => $result]);
    ...
    public static function instance() {
        return new \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\Service();
    }
}
