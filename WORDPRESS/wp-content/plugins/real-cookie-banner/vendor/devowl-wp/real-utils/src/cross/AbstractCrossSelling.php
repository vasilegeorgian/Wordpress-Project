...
abstract class AbstractCrossSelling {
    ...
    public function actionCounter($action, $increment = \false) {
        $optionName =
            \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_CROSS_COUNTER .
            '.' .
            $this->getSlug() .
            '.' .
            $action;
        $cnt = \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::get(
            \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_INITIATOR_CROSS,
            $optionName,
            0
        );
        if ($increment) {
    ...
    public function forceHide($action, $force = \false) {
        $skip = \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::get(
            \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_INITIATOR_CROSS,
            \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_CROSS_SKIP,
            []
        );
        if ($force) {
    ...
    public function dismiss($action, $force) {
        ...
        if (
            \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::set(
                \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_INITIATOR_CROSS,
                \DevOwl\RealCookieBanner\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_NEXT_CROSS_SELLING,
                $ts
            )
        ) {
            return $ts;
