...
class StackCalculator {
    ...
    public function calculateBestSuitableStacks() {
        ...
        foreach (self::STACK_DECLARATIONS as $declaration) {
            $calculated = \array_unique(
                \array_merge($calculated, \array_values($usedDeclarationsToStack[$declaration] ?? []))
            );
        }
    ...
    protected function sortByScore(&$stacks, $truncateScores = \false) {
        \array_multisort(
            \array_column($stacks, 'score'),
            \SORT_DESC,
            \array_column($stacks, 'possibleScore'),
            \SORT_ASC,
            $stacks
        );
        if ($truncateScores) {
