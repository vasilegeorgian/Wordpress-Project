...
class NavMenuList {
    ...
    public function getById($id, $checkAlreadyAppliedBy) {
        ...
        return \array_values(
            \array_filter($nav_menus, function ($nav) use ($id) {
                return $nav['id'] === $id;
            })
        )[0] ?? null;
    }
    ...
    public function get($checkAlreadyAppliedBy = null) {
        ...
        foreach ($nav_menus as $nav_menu) {
            $row = [
                'id' => \md5('legacy_nav' . $nav_menu->term_id),
                'type' => 'legacy_nav',
                'term_id' => $nav_menu->term_id,
                'name' => $nav_menu->name,
                'locations' => new \stdClass(),
                'languages' => $this->getLanguagesForMenu($nav_menu),
                'edit_url' => admin_url(\sprintf('nav-menus.php?action=edit&menu=%d', $nav_menu->term_id))
            ];
            // Find location
            ...
            if (\is_callable($checkAlreadyAppliedBy)) {
                $row['applied'] =
                    \count(
                        \array_filter(wp_get_nav_menu_items($nav_menu, ['nopaging' => \true]), $checkAlreadyAppliedBy)
                    ) > 0;
            }
            $result[] = $row;
        }
    ...
    protected function getLanguagesForMenu($menu) {
        $result = [];
        $compLanguage = $this->compInstance();
        if ($compLanguage instanceof \DevOwl\RealCookieBanner\Vendor\DevOwl\Multilingual\PolyLang) {
                ...
                foreach ($options['nav_menus'][$theme] as $location => $languages) {
                    foreach ($languages as $language => $language_menu_id) {
                        if ($language_menu_id === $menu->term_id) {
                            $result[] = [
                                'code' => $language,
                                'language' => $compLanguage->getTranslatedName($language),
                                'location' => $location
                            ];
                        }
                    }
        ...
        } elseif ($compLanguage instanceof \DevOwl\RealCookieBanner\Vendor\DevOwl\Multilingual\WPML) {
            $nav_details = apply_filters('wpml_element_language_details', null, [
            ...
            if ($nav_details !== null) {
                ...
                $result[] = [
                    'code' => $language_code,
                    'language' => $compLanguage->getTranslatedName($language_code),
                    'locationOf' => $compLanguage->getOriginalTermId($menu->term_id, 'nav_menu')
                ];
            }
        }
        return $result;
