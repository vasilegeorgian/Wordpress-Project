...
class ContrastRatioValidator {
    ...
    public function validate_callback($validity) {
        ...
        $against1 = \DevOwl\RealCookieBanner\Vendor\DevOwl\Customize\Utils::getValue($this->setting1, $this->panel);
        $against2 = \DevOwl\RealCookieBanner\Vendor\DevOwl\Customize\Utils::getValue($this->setting2, $this->panel);
        ...
        if ($ratio < $this->minimum) {
            $validity->add(
                ...
                \sprintf($this->message, number_format_i18n($ratio, 3), $this->minimum),
                ['color1' => $against1, 'color2' => $against2]
            );
        }
        return $validity;
    }
