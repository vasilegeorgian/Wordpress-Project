...
class RpmInitiator extends \DevOwl\RealCookieBanner\Vendor\DevOwl\RealProductManagerWpClient\AbstractInitiator {
    ...
    public function buildTelemetryData($telemetry) {
        ...
        $countryBypassSettings = \DevOwl\RealCookieBanner\settings\CountryBypass::getInstance();
        $tcfSettings = \DevOwl\RealCookieBanner\settings\TCF::getInstance();
        $multisiteSettings = \DevOwl\RealCookieBanner\settings\Multisite::getInstance();
            ...
            ->add(
                ...
                \DevOwl\RealCookieBanner\Utils::array_flatten(
                    \array_map(function ($group) {
                        ...
                        return $items;
                    }, $revisionCurrent['revision']['groups'])
                )
            )
            ->add(
                'rcb_contentBlockers',
                \array_map(function ($blocker) {
                    ...
                    return $item;
                }, $revisionIndependent['revision']['blocker'])
            )
            ->add(
        ...
        $telemetry->add(
            'rcb_consent_count',
            \DevOwl\RealCookieBanner\UserConsent::getInstance()->byCriteria(
                [],
        ...
        $telemetry
            ...
            ->add('rcb_settings_ePrivacyUSA', $consentSettings->isEPrivacyUSAEnabled())
