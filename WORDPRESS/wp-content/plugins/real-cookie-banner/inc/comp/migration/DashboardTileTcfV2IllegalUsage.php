...
class DashboardTileTcfV2IllegalUsage extends \DevOwl\RealCookieBanner\comp\migration\AbstractDashboardTileMigration {
        ...
        $this->addAction(
            ...
            \sprintf(
                ...
                \join('</p><p>', [
                    ...
                    __(
                        'If you have previously obtained consents under the Transparency & Consent Framework, be sure to read the following additional information and consult a legal advisor to clarify the further procedure for your individual case.',
                        RCB_TD
                    ),
                    \sprintf(
                        '<ul>
    ...
    <li><a href="%s" target="_blank">%s</a></li>
            ...
            ['linkText' => __('Disable TCF', RCB_TD), 'callback' => $this->getConfigUrl('/settings/tcf')]
        );
    }
    // Documented in AbstractDashboardTileMigration
    ...
    public function getDescription() {
        return __(
            'We need to inform you that on Feb. 02, 2022, the Belgian Data Protection Authority (APD) declared the collection of consents under the Transparency & Consent Framework (TCF) to be invalid. The decision is effective across the EU as it was issued in a one-stop-shop procedure.',
            RCB_TD
        );
    }
    // Documented in AbstractDashboardTileMigration
    public function isActive() {
        ...
        $previousVersions = \DevOwl\RealCookieBanner\Core::getInstance()
            ->getActivator()
    ...
    public function dismiss() {
        return \DevOwl\RealCookieBanner\Core::getInstance()
            ->getActivator()
            ->removePreviousPersistedVersions(function ($v) {
