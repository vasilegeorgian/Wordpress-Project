	...
	class Addify_Registration_Fields_Addon {
		...
		public function afreg_installation() {
			...
			if (!is_dir($upload_url['basedir'] . '/addify_registration_uploads')) {
				...
				$fh = fopen( $upload_url['basedir'] . '/addify_registration_uploads/index.html', 'w+' ) || die( "Couldn't open file" );
				fwrite( $fh, '<html><head /><body><h1>You are not allowed to access this folder!</h1></html>' );
				fclose( $fh );
			}
		...
		public function get_states() {
			...
			if (!empty($_POST['placeholder'])) {
				$placeholder = __(sanitize_text_field($_POST['placeholder']), 'addify_reg');
			} else {
				$placeholder = __('Select a county / state...', 'addify_reg');
			}
