...
if ( !class_exists( 'Addify_Registration_Fields_Disapproved_User_Email' ) ) { 

	class Addify_Registration_Fields_Disapproved_User_Email extends WC_Email {

		/**
		 * Constructor of membership activated.
		 */
		public function __construct() { 
			$this->id             = 'afreg_disapproved_user_email_user'; // Unique ID to Store Emails Settings
			$this->title          = __( 'Addify Registration Disapproved User Email to Customer', 'addify_reg' ); // Title of email to show in Settings
			$this->customer_email = true; // Set true for customer email and false for admin email.
			$this->description    = __( 'This email will be sent to customer when account is disapproved.', 'addify_reg' ); // description of email
			$this->template_base  = AFREG_PLUGIN_DIR . 'templates/'; // Base directory of template 
			$this->template_html  = 'emails/afreg-disapproved-user-email-user.php'; // HTML template path
			$this->template_plain = 'emails/plain/afreg-disapproved-user-email-user.php'; // Plain template path
			...
			// Trigger function.
			add_action( 'afreg_disapproved_user_email_notification_user', array( $this, 'trigger' ), 10, 1 ); // action hook(s) to trigger email 
		...
		public function get_default_subject() {
			...
			if (!empty(get_option('afreg_disapproved_email_subject'))) {

				return __(get_option('afreg_disapproved_email_subject'), 'addify_reg');

			} else {

				return __( 'Sorry your {site_title} account has been disapproved!', 'addify_reg' );
		...
		public function get_default_heading() {
			return __( 'Account disapproved on {site_title}', 'addify_reg' );
		}
		...
		public function trigger( $customer_id ) {
			...
			if (!empty($customer)) {
				...
				//custom message
				$email_content = get_option('afreg_disapproved_email_text');
