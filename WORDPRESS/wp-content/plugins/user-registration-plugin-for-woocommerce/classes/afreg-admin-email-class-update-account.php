...
if ( !class_exists( 'Addify_Registration_Fields_Admin_Email_Update_Account' ) ) { 

	class Addify_Registration_Fields_Admin_Email_Update_Account extends WC_Email {

		/**
		 * Constructor of membership activated.
		 */
		public function __construct() { 
			$this->id             = 'afreg_update_user_email_admin'; // Unique ID to Store Emails Settings
			$this->title          = __( 'Addify Registration Update User Email Admin', 'addify_reg' ); // Title of email to show in Settings
			$this->customer_email = false; // Set true for customer email and false for admin email.
			$this->description    = __( 'This email will be sent to admin when user update account.', 'addify_reg' ); // description of email
			$this->template_base  = AFREG_PLUGIN_DIR . 'templates/'; // Base directory of template 
			$this->template_html  = 'emails/afreg-update-user-email-admin.php'; // HTML template path
			$this->template_plain = 'emails/plain/afreg-update-user-email-admin.php'; // Plain template path
			...
			// Trigger function.
			add_action( 'afreg_update_user_email_notification_admin', array( $this, 'trigger' ), 10, 2 ); // action hook(s) to trigger email 
		...
		public function get_default_subject() {
			...
			return __( '[{site_title}]: User Updated', 'addify_reg' );
		}
		...
		public function get_default_heading() {
			return __( 'User Updated', 'addify_reg' );
		}


		public function trigger( $customer_id, $new_user_role ) {
			...
			if (!empty($customer)) {
				...
				//custom message
				$email_content = get_option('afreg_update_user_admin_email_text');
				...
				$customer_details .= '<p><b>' . esc_html__( $role_field_label . ': ', 'addify_reg') . '</b><span style="text-transform: capitalize;">' . $new_user_role . '</span></p>';
