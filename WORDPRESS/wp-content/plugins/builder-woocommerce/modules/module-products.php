...
class TB_Products_Module extends Themify_Builder_Component_Module {
	...
	function get_assets() {
		$arr= array(
			'css' => themify_enque(Builder_Woocommerce::$url . 'assets/'.$this->slug.'.css'),
			'js' => themify_enque( Builder_Woocommerce::$url . 'assets/scripts.js' ),
			'ver' => Builder_Woocommerce::$version
		);
		if(is_rtl()){
			$arr['css']=array(
				$this->slug=>$arr['css'],
				$this->slug.'_rtl'=>themify_enque(Builder_Woocommerce::$url . 'assets/modules/rtl.css')
			);
		}
		return $arr;
	}
	...
	public function get_options() {
		return array(
            ...
            array(
                'id' => 'title_tag',
                'type' => 'select',
                'label' => __('Product Title Tag', 'themify'),
                'h_tags' => true,
                'default' => 'h3'
            ),
            array(
