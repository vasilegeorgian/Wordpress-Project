...
class TB_Fancy_Heading_Module extends Themify_Builder_Component_Module {
	...
	$fh_dvdr_icon = array(
		...
		self::get_tab(array(
			'n' => array(
				'options' => array(
					self::get_margin(array('.module .tb_fancy_heading_icon em', '.module .tb_fancy_heading_icon img'), 'fh_d_i_m'),
				)
			),
			'h' => array(
				'options' => array(
					self::get_margin(array('.module:hover .tb_fancy_heading_icon em', '.module:hover .tb_fancy_heading_icon img'), 'fh_d_i_m_h'),
				)
			)
	    ))
		)),
	...
	$fh_dvdr_border = array(
		...
		self::get_tab(array(
			...
			'h' => array(
				'options' => array(
					self::get_width('.module:hover .tb_fancy_heading_border', 'fh_d_b_w_h'),
				)
			)
	    ))
		)),
    ...
    <# var inline=data.inline_text==='1'?' inline-fancy-heading':'',
		...
		has_icon = icon !== '' || image !== '',
		icon_style = data.icon_c ? 'style="color: ' + tb_app.Utils.toRGBA( data.icon_c ) + '"' : '';
	#>
	<div class="module module-<?php echo $this->slug; ?> {{ data.css_class }}{{ inline }}{{divider}}">
		...
		<# if ( has_icon ) { #>
			<span class="tb_fancy_heading_icon_wrap {{inline}}">
				<# if ( ! divider ) { #><span class="tb_fancy_heading_border"></span><# } #>
				<span class="tb_fancy_heading_icon">
					<# if ( icon_type === 'icon' && icon !== '' ) { #>
						<em {{{icon_style}}}><# print(tb_app.Utils.getIcon( icon ).outerHTML) #></em>
					<# } else if ( image ) { #>
						<img src="{{image}}">
					<# } #>
				</span>
