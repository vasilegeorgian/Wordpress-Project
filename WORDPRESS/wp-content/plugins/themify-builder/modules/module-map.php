...
class TB_Map_Module extends Themify_Builder_Component_Module {
	...
	public function __construct() {
		...
		add_filter( 'themify_builder_ajax_front_vars', array(__CLASS__, 'check_map_api'));
		add_filter( 'themify_builder_ajax_admin_vars', array(__CLASS__, 'check_map_api'));
	}
    ...
    style = '';
    if(data.b_width_map){
    var b_type=!data.b_style_map?'solid':data.b_style_map;
    style+= 'border: '+b_type+' '+data.b_width_map+'px';
    if (data.b_color_map) {
	...
	scroll = data.scrollwheel_map === 'enable'?1:0,
	drag = data.draggable_map === 'enable'?1:0,
	mdrag=data.draggable_disable_mobile_map=='yes'?1:0,
	control = data.map_control === 'no' ? 1 : 0,
	type= data.map_provider === 'google'?data.type_map:data.bing_type_map;
	#>
	<div data-map-provider="{{ data.map_provider }}" data-address="{{ address }}" data-control="{{control}}" data-zoom="{{ data.zoom_map }}" data-type="{{ type }}" data-scroll="{{ data.scroll }}" data-drag="{{ data.drag }}" data-mdrag="{{ data.mdrag }}" class="themify_map<# print(data.map_provider != 'google'?' themify_bing_map':'')#>"  style="{{ style }}"  data-info-window="{{ info }}" data-reverse-geocoding="{{ reverse }}"></div>
	<# } #>
