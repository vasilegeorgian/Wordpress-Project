...
<div <?php echo self::get_element_attributes( self::sticky_element_props( $container_props, $fields_args ) ); ?>>
    ...
    <form class="tb_signup_form" name="tb_signup_form">
        ...
        <div>
            <label>
                <span class="tb_signup_label"<?php self::add_inline_edit_fields('l_name')?>><?php echo esc_html( $fields_args['l_name'] ) ?></span>
            </label>
            <div class="tf_box tf_w row_inner tf_clearfix">
                <div class="module_column col2-1 first">
                    <label>
                        <input type="text" name="first_n">
                        <span<?php self::add_inline_edit_fields('l_firstname')?>><?php echo esc_html( $fields_args['l_firstname'] ) ?></span>
                    </label>
                </div>
                <div class="module_column col2-1 last">
                    <label>
                        <input type="text" name="last_n">
                        <span<?php self::add_inline_edit_fields('l_lastname')?>><?php echo esc_html( $fields_args['l_lastname'] ) ?></span>
                    </label>
                </div>
            </div>
        </div>
        <div>
            <label>
                <span class="tb_signup_label" data-required="yes"<?php self::add_inline_edit_fields('l_username')?>><?php echo esc_html( $fields_args['l_username'] ) ?></span>
                <input type="text" name="usr">
            </label>
        </div>
        <div>
            <label>
                <span class="tb_signup_label" data-required="yes"<?php self::add_inline_edit_fields('l_email')?>><?php echo esc_html( $fields_args['l_email'] ) ?></span>
                <input type="email" name="email">
            </label>
        </div>
        <div>
            <label>
                <span class="tb_signup_label" data-required="yes"<?php self::add_inline_edit_fields('l_password')?>><?php echo esc_html( $fields_args['l_password'] ) ?></span>
                <input type="password" name="pwd">
            </label>
        </div>
        <div>
            <label>
                <span class="tb_signup_label"<?php self::add_inline_edit_fields('l_bio')?>><?php echo esc_html( $fields_args['l_bio'] ) ?></span>
                <textarea name="bio"></textarea>
            </label>
			<?php if ( $fields_args['desc'] !== '' ): ?>
                <p<?php self::add_inline_edit_fields('desc')?>><?php echo esc_html( $fields_args['desc'] ) ?></p>
			<?php endif; ?>
        </div>
        ...
        <button name="tb_submit"<?php self::add_inline_edit_fields('l_submit')?>><?php echo esc_html( $fields_args['l_submit'] ) ?></button>
