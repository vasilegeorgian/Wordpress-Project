...
<div <?php echo self::get_element_attributes(self::sticky_element_props($container_props,$fields_args)); ?>>
		...
		<?php foreach( $tweets as $tweet ) : ?>
			<li class="tb_twitter_item">
				...
				<?php if ( $fields_args['time'] ) : ?>
					<br /><em class="tb_twitter_timestamp"><small><?php echo sprintf( __( '%s ago', 'themify' ), human_time_diff( strtotime( $tweet->created_at ) ) ); ?></small></em>
				<?php endif; ?>
			</li>
		<?php endforeach; ?>
	</ul>
	...
	<?php if ( $fields_args['show_follow'] ) : ?>
		<a class="tb_twitter_follow" href="<?php echo esc_url( '//twitter.com/' .  $fields_args['username'] ); ?>"><?php echo esc_html( $fields_args['follow'] ); ?></a>
	<?php endif; ?>
