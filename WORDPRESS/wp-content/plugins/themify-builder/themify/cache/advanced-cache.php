...
if(isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD']==='GET' && !function_exists('themify_cache_loaded') && !empty($_SERVER['REQUEST_URI']) && empty( preg_grep( '/wordpress_logged_in_/', array_keys( $_COOKIE ) ) )){
    ...
    if(!((defined( 'DOING_AJAX' ) && DOING_AJAX) || (defined( 'XMLRPC_REQUEST' ) && XMLRPC_REQUEST) || (defined( 'DOING_CRON' ) && DOING_CRON) || (defined( 'REST_REQUEST' ) && REST_REQUEST) || (defined('APP_REQUEST') && APP_REQUEST) || (defined( 'SHORTINIT' ) && SHORTINIT) || strpos( $_SERVER['REQUEST_URI'], 'robots.txt' ) !== false || strpos( $_SERVER['REQUEST_URI'], 'wp-admin' )!==false || strpos( $_SERVER['REQUEST_URI'], 'wp-login' )!==false)){
