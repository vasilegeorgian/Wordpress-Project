...
class Themify_Twitter_Api {
	...
	private function get_bearer_token() {
		...
		add_option( self::TOKEN_KEY, $result->access_token, '', false );
	...
	public function query( $query, $settings = [] ) {
		...
		if ( ! $settings['disable_cache'] ) {
			$transient_id = md5( implode( '', $query ) );
		...
		if ( ! $this->bearer_token = get_option( self::TOKEN_KEY ) ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			return new WP_Error( 'bad_query', __( 'Bad response from Twitter. Check your query.', 'themify' ) );
		}

		$data = json_decode( $response['body'] );
		if ( ! $settings['disable_cache'] ) {
			$cache_duration = empty( $settings['cache_duration'] ) ? 10 : (int) $settings['cache_duration'];
			set_transient( $transient_key, $data, $cache_duration * 60 );
		}
