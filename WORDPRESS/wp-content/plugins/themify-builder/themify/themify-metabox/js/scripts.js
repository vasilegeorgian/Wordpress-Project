...
window.Themify_Metabox = (function($){
	...
	api.builder_page = function () {
		const el = document.getElementById('builder_page');
		if(el){
			const layout = document.querySelector('#page-options [alt="sidebar-none"]').parentNode,
				content = document.querySelector('#page-options [alt="full_width"]').parentNode,
				layoutRow = layout.closest('.themify_field_row'),
				contentRow = content.closest('.themify_field_row'),
				hideTitle = document.getElementById('hide_page_title'),
				hideTitleRow = hideTitle.closest('.themify_field_row'),
				rows = [layoutRow, contentRow, hideTitleRow];
