(function ($) {
  ...
  // Public methods
  $.extend($.fn, {
    tfminicolors: function(method, data) {
  ...
  function init(input, settings) {
        ...
        // The swatch
        if(!settings.inline) {
            input.after('<span class="tfminicolors-swatch tfminicolors-sprite tfminicolors-input-swatch"><span class="tfminicolors-swatch-color tf_abs"></span></span>');
        }
        input.on('focus.tfminicolors blur.tfminicolors keydown.tfminicolors keyup.tfminicolors paste.tfminicolors',function(event){
            ...
            if(type==='focus' && !input.parent().hasClass('tfminicolors-var-input')){
                ...
                // Set color string
                if(inputVal !== '' && (inputVal.indexOf('--')===0 || $.inArray(inputVal.toLowerCase(), keywords) > -1)) {
                  value = inputVal;
                } else {
        ...
        .closest('.tfminicolors').on('mousedown.tfminicolors touchstart.tfminicolors', '.tfminicolors-grid, .tfminicolors-slider, .tfminicolors-opacity-slider', function(e) {
            if(e.which===1){
                e.preventDefault();
                const target = $(this),
                ...
                move(target, e, true);
                $(d).on('mousemove.tfminicolors touchmove.tfminicolors', function(e) {
                        move(target, e);
                  })
                  // Stop moving
                .one('mouseup.tfminicolors touchend.tfminicolors', function() {
                    $(d).off('mousemove.tfminicolors touchmove.tfminicolors');
                });
            }
        })
        .on('click.tfminicolors','.tfminicolors-swatches li',function(e){
                e.preventDefault();
    ...
    if(settings.swatches && settings.swatches.length !== 0) {
      ...
      for(i = 0; i < settings.swatches.length; ++i) {
        ...
        $('<li class="tfminicolors-swatch tfminicolors-sprite"><span class="tfminicolors-swatch-color tf_abs" title="' + name + '"></span></li>')
          .appendTo(swatches)
    ...
    window.themifyColorManager = {
        ...
        initColorPicker(el) {
            ...
            const newSwatchesContainer = document.createElement('div'),
                ...
                addSwatch = document.createElement('div'),
        ...
        // Import Colors
        importColors(type,doc){
            doc = doc || document;
            let input = doc.getElementsByClassName('themify_cm_input')[0];
            if(!input){
        ...
        // Add a swatch
        addSwatch(doc){
            const input = doc.querySelector('.tfminicolors-focus .tfminicolors-input'),
			...
			const UID = this.UID();
			this.addSwatchHtml(doc,newColor,newOpacity,UID);
			this.colorSwatches[UID] = {color:newColor,opacity:newOpacity,uid:UID};
			// update the color swatches
			this.updateColorSwatches('colors');
        },
        // Add Swatche HTML
        addSwatchHtml(doc,color,opacity,UID){
            ...
            for(let containers=doc.querySelectorAll('.tf_swatches_container .tfminicolors-swatches'),i=containers.length-1; i >-1; --i ){
