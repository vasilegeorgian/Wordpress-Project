(function ($, Themify, window, topWindow, document, api, Common) {
	...
	function renameProp(p) {
        ...
        ThemifyLiveStyling.prototype.addOrRemoveFrame = function (_this, settings) {
			...
			if ( ! Themify.cssLazy['tf_frames'] ) {
				Themify.cssLazy['tf_frames'] = true;
				Themify.LoadCss( themifyBuilder.builder_url + '/css/modules/frames.css' );
			}
			if ( ! frame_wrap ) {
    ...
    api.EdgeDrag = {
        ...
        getCurrentStyling(id,model,type) {
            ...
            if(v!==undefined && v!=='' && slug==='column' && v.toString().indexOf(',')!==-1 && (id.indexOf('padding')===0 || id.indexOf('margin'))){
                v=v.trim().split(',');
