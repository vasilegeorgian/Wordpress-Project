        ...
        clone(el) {
            ...
            if (tb_app.mode === 'visual') {
                ...
                if (v.length > 0) {
                    ...
                    for (let items=node.getElementsByClassName('big-video-wrap'),i=items.length-1;i>-1;--i) {
                        if (items[i]) {
                            items[i].remove();
                        }
                    }
                }
				for (let items=node.getElementsByClassName('tb_dragger'),i=items.length-1; i>-1;--i) {
					items[i].remove();
				}
                for (let items=Themify.selectWithParent('tb_element_clicked',node),i=items.length-1; i>-1;--i) {
        ...
        Lightbox: {
            ...
            fixContainment() {
                 const box = this.$lightbox[0].getBoundingClientRect(),
						el_w=box.width,
                        w = topWindow.innerWidth - 20,
                        h = topWindow.innerHeight - 30,
                        left = box.left,
                        top = box.top,
						new_pos = {};
            ...
            open(options, beforeCallback, afterCallback) {
                const self = ThemifyBuilderCommon.Lightbox,
                        ...
                        lightboxContainer = self.$lightbox[0].querySelector('#tb_lightbox_container'),
