...
function woof_meta_print_li($m, $meta_types) {
    ?>
                    <li class="woof_options_li">
                        ...
                        <div class="woof_options_item">
                            <input type="text" name="woof_settings[meta_filter][<?php echo esc_attr($m['meta_key']) ?>][title]" placeholder="<?php esc_html_e('enter title', 'woocommerce-products-filter') ?>" value="<?php esc_html_e($m['title']) ?>" class="woof_column_li_option woof_fix2" />
    ...
    foreach ($meta_types as $key => $type):
        ...
        ?> 
                                        <option  <?php selected($m['search_view'], $key) ?> value="<?php esc_html_e($key) ?>" data-show-options="<?php echo esc_attr(($type['show_options']) ? 'yes' : 'no') ?>" data-hideif="<?php esc_html_e(implode(',', $type['hide_if'])) ?>" <?php if (in_array($m['type'], $type['hide_if'])): ?>style="display:none;"<?php endif; ?>>
    ...
    ?>
                        <div class="woof_options_item_options" <?php if (!$show_options): ?> style="display:none;" <?php endif; ?> >
                            <div class="textarea-wrap">
                                <textarea name="woof_settings[meta_filter][<?php echo esc_attr($m['meta_key']) ?>][options]" class="woof_column_li_option" ><?php esc_html_e(isset($m['options']) ? $m['options'] : "") ?></textarea>
                            </div>
