    ...
    <?php if (!empty($terms)): ?>
        <?php foreach ($terms as $term) : $inique_id = uniqid(); ?>
            ...
            <li class="woof_image_term_li_<?php echo esc_attr($term['term_id']) ?> <?php if ($hide_next_term_li): ?>woof_hidden_term<?php endif; ?>">
                <p class="woof_tooltip"><span class="woof_tooltip_data"><?php esc_html_e($term['name']) ?> <?php echo wp_kses_post($count_string) ?><?php if(!empty($term_desc)): ?><br /><i><?php esc_html_e($term_desc) ?></i><?php endif; ?></span>

                    <?php $image_id = 'woof_tax_image_' . $term['slug']; ?>

                    <input id="<?php echo esc_attr($image_id) ?>" type="checkbox" data-styles="<?php esc_html_e($styles) ?>" <?php checked(in_array($term['slug'], $current_request)) ?> id="<?php echo esc_attr('woof_' . $term['term_id'] . '_' . $inique_id) ?>" class="woof_image_term woof_image_term_<?php echo esc_attr($term['term_id']) ?> <?php if (in_array($term['slug'], $current_request)): ?>checked<?php endif; ?>" data-image="<?php echo esc_url($image) ?>" data-tax="<?php esc_html_e(woof()->check_slug($tax_slug)) ?>" name="<?php esc_html_e($term['slug']) ?>" value="<?php esc_html_e($term['term_id']) ?>" data-term-id="<?php echo esc_attr($term['term_id']) ?>" <?php checked(in_array($term['slug'], $current_request)) ?> /></p>
                <input type="hidden" value="<?php esc_html_e($term['name']) ?>" data-anchor="woof_n_<?php echo esc_attr(woof()->check_slug($tax_slug)) ?>_<?php echo esc_attr($term['slug']) ?>" />
                <label class="woof_wcga_label_hide"  for="<?php echo esc_attr($image_id) ?>"><?php esc_html_e($term['name']); ?></label>
                <?php if (isset(woof()->settings['show_title'][$tax_slug]) AND woof()->settings['show_title'][$tax_slug]): ?>
                    <p class="woof_image_text_term">
                        <?php esc_html_e($term['name']) ?> <?php echo wp_kses_post(wp_unslash($count_string)) ?>
                    </p>
                <?php endif; ?>
            </li>
            <?php
