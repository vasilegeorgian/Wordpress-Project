...
?>
<label class="woof_wcga_label_hide"  for="<?php echo esc_attr($select_id) ?>"><?php esc_html_e(WOOF_HELPER::wpml_translate($taxonomy_info)); ?></label>
    ...
    foreach ($collector as $ts => $values) {
        if (!empty($values)) {
            foreach ($values as $value) {
                ?>
                <input type="hidden" value="<?php esc_html_e($value['name']) ?>" data-anchor="woof_n_<?php echo esc_attr($this->check_slug($ts)) ?>_<?php echo esc_attr($value['slug']) ?>" />
                <?php
            }
        }
    }
