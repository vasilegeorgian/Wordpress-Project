...
<?php if (!empty($price2_filter_data)): ?>
    ...
    <?php foreach ($price2_filter_data['ranges']['options'] as $k => $value): $value = trim($value); ?>
                ...
                <option <?php if ($c == 0 AND $show_count): ?>disabled=""<?php endif; ?> <?php selected($price2_filter_data['selected'], $k); ?> value="<?php echo esc_attr($k) ?>"><?php echo wp_kses_post(wp_unslash($value)) ?> <?php esc_html_e($cs) ?></option>
            <?php endforeach; ?>
