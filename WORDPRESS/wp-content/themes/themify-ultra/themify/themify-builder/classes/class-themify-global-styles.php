...
final class Themify_Global_Styles{
		...
		<?php if ( !empty( $globalStyles ) ): ?>
			<?php foreach ( $globalStyles as $style ): ?>
				...
				if ( 'publish' === $page_status ): ?>
									<a href="<?php echo self::export_url($style['id']); ?>" target="_blank" class="tb_gs_export tb_export" data-id="<?php echo $style['id']; ?>"><?php echo themify_get_icon('export','ti'); ?></a>
				<?php else: ?>
	...
	public static function localize_data($vars) {
		...
		if(self::$isGlobalEditPage===true) {
			$vars['globalStyleData']=array('id'=>$post_id, 'class'=>get_post_field('post_name', $post_id), 'type'=>get_post_meta($post_id, 'themify_global_style_type', true), 'exportURL'=>self::export_url($post_id));
		} else {
