...
class TB_Tab_Module extends Themify_Builder_Component_Module {
	...
	?>
    <# var color = undefined === data.color_tab || 'default' == data.color_tab ? 'tb_default_color' : data.color_tab, 
	...
	<div class="module module-<?php echo $this->slug; ?> ui tab-style-{{ data.style_tab }} {{ data.layout_tab }} {{ color }} {{ data.css_tab }} <# data.tab_appearance_tab  ? print( data.tab_appearance_tab.split('|').join(' ') ) : ''; #>" <# ( 'allow_tab' == data.allow_tab_breakpoint && '' != data.tab_breakpoint ) ? print( "data-tab-breakpoint='"+ data.tab_breakpoint +"'" ) : ""; #>  >
		...
		if ( data.tab_content_tab ) {#>
			...
			<div class="tab-nav-current-active tf_hide">
				...
				<span class="tb_tab_title" contenteditable="false" data-name="title_tab" data-repeat="tab_content_tab" data-index="0">{{{ data.tab_content_tab[0].title_tab }}}</span>
			</div>
		<ul class="tab-nav tf_clearfix">
					...
					<li class="<# i == 0 && print('current') #>" aria-expanded="{{i == 0}}">
						<a href="#tab-{{ tabId }}-{{ i }}">
							<# if ( item ) {
							if ( item.icon_tab ) { #><em><# print(tb_app.Utils.getIcon(item.icon_tab).outerHTML)#></em><# }
				...
				<# _.each( data.tab_content_tab, function( item,i ) { #>
					<div data-id="tab-{{ tabId }}-{{ i }}" class="tab-content" aria-hidden="{{i != 0}}">
						<div class="tb_text_wrap" contenteditable="false" data-name="text_tab" data-index="{{i}}" data-hasEditor data-repeat="tab_content_tab"><# item && item.text_tab && print( item.text_tab ) #></div>
					</div>
				<# } ); #>
			</div>
	    <# } #>
