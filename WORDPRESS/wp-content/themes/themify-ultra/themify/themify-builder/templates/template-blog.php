...
<?php if('yes'!==$fields_args['hide_page_nav_' . $mod_name]): ?>
    <?php if('ajax'===$fields_args['nav_type']): ?>
        ...
        if($the_query->max_num_pages > $args['paged']) {
            ...
            $url=next_posts($the_query->max_num_pages, false);
            echo '<p class="tf_load_more tf_textc tf_clear"><a data-url="' . $url . '" data-id="' . $element_id . '" href="' . $url . '" data-page="' . esc_attr($args['paged']) . '" class="load-more-button">' . __('Load More', 'themify') . '</a></p>';
        }
        ?>
    <?php else: ?>
