...
class Themify_Enqueue_Assets {
    ...
    public static function loadGoogleFonts($content = '') {
        ...
        if (!empty($res)) {
            $fonts = array();
            foreach ( $res as $font_name => $variants ) {
				foreach ( $variants as $i => $variant_name ) {
					if ( strpos( $variant_name, 'i' ) !== false ) {
						/* for Italic fontWeights, make sure normal variant is loaded too */
						$normal_variant = (int) $variant_name;
						if ( ! in_array( $normal_variant, $variants ) ) {
							$variants[ $i ] .= ',' . $normal_variant;
						}
					} else {
						/* for normal variants, load italic fontWeight too */
						if ( ! in_array( $variant_name . 'i', $variants ) ) {
							$variants[ $i ] .= ',' . $variant_name . 'i';
						}
					}
				}
                $fonts[] = $font_name . ':' . implode( ',', array_unique( $variants ) );
            }
            $res = null;
