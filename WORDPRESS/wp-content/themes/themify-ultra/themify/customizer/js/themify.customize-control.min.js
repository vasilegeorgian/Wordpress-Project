...
a.editor.codemirror.on("keydown",function(f,g){27==g.keyCode&&g.stopPropagation()});h("customcss",function(f){f.validate=function(g){const e=wp.CodeMirror.lint.css(g,{errors:1});e&&0<e.length?f.notifications.add("invalid_css",new wp.customize.Notification("invalid_css",{message:themifyCustomizerControls.invalid_css,type:"warning"})):f.notifications.remove("invalid_css");return g}})}}});h.controlConstructor.themify_customcss=h.ThemifyCustomCSS;h.ThemifyImageSelect=h.ThemifyControl.extend({field:".themify_image_select_control",
ready(){var b=this,a=c(b.field,b.container);a.val()?b.value[b.id]=m(a.val()):b.value[b.id]={type:"",selected:""};c(".image-select",b.container).on("click",function(d){d.preventDefault();b.value[b.id].selected=c(this).data("option");a.val(JSON.stringify(b.value[b.id])).trigger("change")})}});h.controlConstructor.themify_image_select=h.ThemifyImageSelect;h.ThemifyTools=h.ThemifyControl.extend({field:".themify_tools_control",clearing:!1,saveOptionCallback(){c("#customize-header-actions").find(".save").trigger("click")},
ready(){var b=this;h.bind("saved",function(){if(h.ThemifyTools.field){if("true"===b.getParameterByName("cleared"))window.location.reload();else{var a=window.top.location.href;a="#"==a[a.length-1]?a.replace("#",""):a;window.top.location=a}h.ThemifyTools.field=!1}});c(".clearall",b.container).on("click",function(){if(window.confirm(themifyCustomizerControls.clearMessage)){h.ThemifyTools.field=!0;c(".themify-customizer-value-field").each(function(){c(this).val("").trigger("change")});h.previewer.send("themify-customize-clearall");
var a=c(".customize-control-themify_logo").find(".site-name").val(),d=c(".customize-control-themify_tagline").find(".site-name").val(),f=c(this).data("sitename"),g=c(this).data("tagline");a!=f||d!=g?(b.saveOption("blogname",f),b.saveOption("blogdescription",g)):b.saveOptionCallback()}});c(".customize-export",b.container).on("click",function(a){"Published"!=c("#customize-header-actions").find(".save").val()&&(window.confirm(themifyCustomizerControls.exportMessage)||a.preventDefault())})}});h.controlConstructor.themify_tools=
h.ThemifyTools;(function(){var b,a;c("body").on("click","#customize-footer-actions .devices button",function(){var d=c(this).data("device"),f=c("#customize-preview");"object"===typeof themifyCustomizerControls.responsiveBreakpoints&&(b=themifyCustomizerControls.responsiveBreakpoints[d])?("tablet_landscape"===d&&(b=976),a={width:b+"px",marginLeft:-b/2}):a={width:"",marginLeft:""};f.css(a)})})();c("body").on("click","#customize-control-start_mobile_menu_acc_ctrl > .themify-suba-toggle",function(b){var a=
c("#customize-preview > iframe")[0].contentWindow;c(this).closest(".customize-control").hasClass("topen")?(c("#customize-footer-actions .preview-desktop").click(),setTimeout(function(){c("#customize-preview").width()>themifyCustomizerControls.mobile_menu_trigger_point&&c("#customize-preview").css({width:themifyCustomizerControls.mobile_menu_trigger_point+"px",marginLeft:(c("#customize-preview").width()-themifyCustomizerControls.mobile_menu_trigger_point)/2});setTimeout(function(){a.jQuery("body").hasClass("mobile-menu-visible")||
a.jQuery("#menu-icon span").click()},1E3)},1E3)):c("#customize-footer-actions .preview-desktop").click()}).on("click","#customize-footer-actions button",function(){c(this).hasClass("preview-desktop")||c("#customize-control-start_mobile_menu_acc_ctrl.topen .themify-suba-toggle").click()})})(wp,jQuery);
