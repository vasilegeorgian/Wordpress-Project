    ...
    if (!empty($img_url)) {
        ...
        if ($args['urlonly']) {
            ...
            if ($attachment_id && $args['disable_responsive'] === false) {
                $out = function_exists('wp_filter_content_tags') ? wp_filter_content_tags($out) // WP 5.5
    ...
    if (isset($html) && (strpos($html, 'src=') !== false || strpos($html, '<audio') !== false)) {
        ...
        if (!empty($matches[0])) {
            ...
            foreach ($matches as $item) {
                if (!empty($item) && strpos($item, 'data-lazy', 4) === false && strpos($item, 'data-tf-not-load', 4) === false && strpos($item, 'data-tf-src', 4) === false) {
