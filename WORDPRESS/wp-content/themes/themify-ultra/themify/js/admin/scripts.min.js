...
var e=[],f=0;void 0!==b.data.actions&&a.each(b.data.actions,function(g,h){a.each(h,function(m,r){e.push({action:"themify_import_sample_actions",_action:g,nonce:themify_js_vars.nonce,data:r})})});if(0===e.length)c();else for(b=0;5>b;++b)d()}function p(){a(".tb_alert").addClass("busy").fadeIn(800)}function l(b){"error"===b?(b="error",a(".overlay, .prompt-box").delay(900).fadeIn(500),a(".prompt-box .show-error").show(),a(".prompt-box .show-error p").remove(),a(".prompt-box .error-msg").after('<p class="prompt-error">undefined</p>'),
...
l();window.location=themify_js_vars.admin_url};g.success?L(g,h):(l(),alert(g.data.error))},beforeSend:function(){c.find("span").text(c.attr("data-importing"));p()}})};b.find(".proceed-import").off().on("click",function(e){e.preventDefault();d(a(this).parent().find(".tf_import_images input:checkbox").is(":checked"),a(this).closest("div").find(".erase-demo").is(":checked"))}).end().find("a.external-link").on("click",function(){window.location=a(this).attr("href")})}function W(b,c){const d=a(b).closest(".skin-preview").find(".skin-demo-content").data("skin"),
e=function(f,g){a.ajax({url:ajaxurl,data:{action:"themify_import_sample_content",skin:d,import_images:f?"yes":"no",clear_old_imports:g?"yes":"no",nonce:themify_js_vars.nonce},type:"POST",success:function(h){const m=function(){l();window.location=themify_js_vars.admin_url};h.success?L(h,m):(l(),alert(h.data.error))},beforeSend:function(){p()}})};c.fadeIn().find(".proceed-import").off().on("click",function(f){f.preventDefault();e(a(this).parent().find(".tf_import_images input:checkbox").is(":checked"),
a(this).closest("div").find(".erase-demo").is(":checked"))}).end().find("a.external-link").on("click",function(){window.location=a(this).attr("href");v()})}function O(b){if(!this.dataset.loading){b.preventDefault();var c=this,d=c.classList.contains("themify-updater");c.dataset.loading=!0;!0===d&&(c.dataset.ajax=document.querySelector("#update-main .themify-ajax-installer").dataset.ajax);p();fetch(c.dataset.ajax,{headers:new Headers({"X-Requested-With":"XMLHttpRequest"})}).then(e=>e.text()).then(function(e){delete c.dataset.loading;
...
!0===c;e++)d=b.charAt(e),-1===" 0123456789.+-".indexOf(d)&&(c=!1);c||a(this).val(a(this).val().substring(0,a(this).val().length-1))});a(".save-button").on("click",function(b){J();return!1});if(0<w.length)a(document).on("keydown",function(b){83!=b.which||1!=b.ctrlKey&&1!=b.metaKey||(b.preventDefault(),J())});a("#reset-setting").on("click",function(b){b.preventDefault();b="reset-setting"!==a(this).prop("id")?"styling":"settings";confirm(themify_lang["confirm_reset_"+b])&&(p(),a.post(ajaxurl,{action:"themify_reset_"+
...
n.on("click",".themify-visibility-toggle",function(){var b=a(this),c=b.html();b.html('<span class="tb_slider_loader_wrapper"><span class="tb_slider_loader"></span></span>'+c);q.target=a(this).next();q.showLightbox(a(this).data("item"));return!1}).on("click",".themify_lightbox_visibility .close_lightbox, #themify_lightbox_overlay",function(){q.closeLightbox(a(this).parents(".themify_lightbox_visibility").data("item"));return!1});n.on("click",".themify-visibility-pagination a.page-numbers",function(b){b.preventDefault();
...
nonce:themify_js_vars.nonce},success:function(e){d.innerHTML=e;/^((?!chrome|android).)*safari/i.test(navigator.userAgent)&&(d.blur(),d.dispatchEvent(c))}})},{once:!0,passive:!0})})();N(document.getElementsByClassName("license-link"));(n=document.getElementById("tb_option_flush_twitter"))&&n.addEventListener("click",function(b){b.preventDefault();p();a.ajax({url:ajaxurl,type:"POST",data:{action:"themify_twitter_flush",nonce:themify_js_vars.nonce},success:function(){l()}})});const H=document.getElementById("tf_font_import");
n=document.getElementById("tf_font_size_slider_wrap");H&&(a(H).insertAfter(".wp-header-end"),document.getElementById("tf_font_import_submit").addEventListener("click",function(){p();jQuery.ajax({type:"POST",url:ajaxurl,data:{action:"tf_import_font",nonce:themify_js_vars.nonce,font_name:H.querySelector("select").value},success:function(b){l();b.success?location.reload():console.log(b.data)}})}));if(n){a("#tf_font_size_slider_wrap").appendTo("th#font_preview");let b=document.getElementById("tf_font_preview_style");
n=localStorage.getItem("tf_font_preview_size")||1;let c=function(d){b.innerText=".font_preview span { font-size: "+d+"em; }"};c(n);a("#tf_font_size_slider").slider({value:n,min:.5,max:4,step:.1,slide:function(d,e){c(e.value);localStorage.setItem("tf_font_preview_size",e.value)}})}let x=document.getElementsByClassName("generate-child-theme");for(let b=0;b<x.length;b++)x[b].addEventListener("click",c=>{c.preventDefault();window.confirm(x[b].dataset.confirm)&&(p(),a.ajax({url:ajaxurl,type:"POST",data:{action:"themify_generate_child_theme",
nonce:themify_js_vars.nonce,import_customizer:x[b].closest("div").querySelector('input[type="checkbox"]').checked?1:0},success:d=>{l();d.success?(x[b].classList.add("disabled"),x[b].closest(".tf_generate_child_theme").classList.add("tf_done")):window.alert(d.data[0].message)}}))});a(".themify-ajax-installer").on("click",O);a(function(){const b=a("#themify .preview-icon");b.length&&b.each(function(){var c=a(this),d=c.attr("title");c.removeAttr("title").append('<span class="tm-option-title-fw">'+d+
"</span>")})});a("#themify_feed_settings select").on("change",function(){a(this).next().val(a(this).val().join(","))})})})(jQuery);
