...
function themify_get_placeholder($url,$base=false){
	...
	if(!is_file($destination)){
		...
		$parts=null;
		if ( $ext === 'svg' || $ext === 'svg+xml' || $ext === 'ico' || !function_exists('getimagesize') ) {
			return false;
		}
...
/**
 * Convert image URL to attachment ID, data is cached in a file for faster access
 *
 * @return int|false
 */
function themify_get_attachment_id_cache( $url ) {
	...
	$dir = themify_upload_dir('basedir') . '/tf_image_ids/';
	$found = '';
	$len = strlen( $k ) - 1;
	clearstatcache();
	for ( $i = 1; $i < 6; ++$i ) { // because every file is can 1mb, 6 means 6mb(~4500*6) in real world it's enough
		...
		if ( is_file( $f ) && ( $data = file_get_contents( $f ) ) ) {
			...
			if ( $start !== false ) {
				$start += $len;
				for ( $j = 1; $j < 20; ++$j ) { //20 is max length of IDs in posts table
	...
	if ( $found !== '' ) {
		return (int) $found;
	} else if ( ! is_dir( $dir ) && ! mkdir( $dir, 0755 ) ) {
		$cache_enabled = false;
		return false;
	}
	...
	file_put_contents( $f, $k . $id, FILE_APPEND | LOCK_EX );
...
function themify_createWebp($url){
	...
	if($orig_ex==='webp' || $orig_ex==='svg' || $orig_ex==='svg+xml' || $orig_ex==='apng' || $orig_ex==='bmp'|| $orig_ex === 'ico'){
	    return $url;
	}
...
function themify_get_image_size($url,$isLocal=false){
	...
	$k=','.sprintf('%s',crc32($url)).':';
	$dir=themify_upload_dir('basedir').'/tf_images_sizes/';
	$found='';
	$len=strlen($k)-1;
        clearstatcache();
	for($i=1;$i<6;++$i){//because every file is can 1mb,6 means 6mb(~4500*6) in real world it's enough
