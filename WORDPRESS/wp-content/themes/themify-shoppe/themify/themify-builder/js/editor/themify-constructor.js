        ...
        getStyleVal(id, bp,vals) {
            ...
            if (id!=='' && api.activeModel !== null) {
                ...
                if (bp === 'desktop' || this.clicked !== 'styling') {
                    v= ( vals !== null && vals[id] !== '' ) ? vals[id] : undefined;
                }
                else{
                    const breakpoints = this.breakpointsReverse;
                    for (let i = breakpoints.indexOf(bp), len = breakpoints.length; i < len; ++i) {
                        if (breakpoints[i] !== 'desktop') {
                            if (vals['breakpoint_' + breakpoints[i]] !== undefined && vals['breakpoint_' + breakpoints[i]][id] !== undefined && vals['breakpoint_' + breakpoints[i]][id] !== '') {
                                v= vals['breakpoint_' + breakpoints[i]][id];
                                break;
                            }
                        }
                        else if (vals[id] !== '') {
                ...
                if ( v !== undefined && v!=='' && api.activeModel && api.activeModel.get('elType')==='column' && v.toString().indexOf(',')!==-1 && ( id.indexOf('padding')===0 || id.indexOf('margin') !== -1 ) ) {
                    v=v.trim().split(',');
                    if(v[0]!==undefined && v[1]!==''){
                        v=v[0];
                    }
        ...
        select: {
            ...
            make_options : function( data,v, self ) {
                ...
                for (let k in options) {
                    ...
                    if ( ( Array.isArray( v ) && v.includes( k ) ) || v === k || (v === undefined && k === data.default)) {
                        opt.selected = true;
                    }
                    d.appendChild(opt);
                }
                ...
                for ( let i = fontFamilies.length - 1; i > -1; --i ) {
                    if ( fontFamilies[i] && this.loaded_fonts.indexOf( fontFamilies[i] ) === -1 && (result.google.indexOf( fontFamilies[i] ) === -1 || result.cf.indexOf( fontFamilies[i] ) === -1) ) {
                        let req = fontFamilies[i].split( ':' ),
							type = self.cf[req[0]] !== undefined ? 'cf' : 'google',
                            weight = ('regular' === req[1] || 'normal' === req[1] || 'italic' === req[1] || parseInt( req[1] )) ? req[1] : '400,700';

						if ( 'google' === type ) {
							/* for google fonts always load the italic variants, in case Italic font style is applied */
							weight = weight.split(',');
							for ( let i = weight.length - 1; i > -1; --i ) {
								if ( weight[i].indexOf( 'i' ) === -1 && ! weight.includes( weight[i] + 'i' ) ) {
									weight.push( weight[i] + 'i' );
								} else if ( weight[i].indexOf( 'i' ) !== -1 && ! weight.includes( String( parseInt( weight[i] ) ) ) ) {
									weight.push( parseInt( weight[ i ] ) );
								}
							}
							weight = weight.join(',');
						}

						let f = req[0].split( ' ' ).join( '+' ) + ':' + weight;
                ...
                (function () {
                    ...
                    self.afterRun.push(function () {
                        ...
                        for (let i = 0, len = opt.length; i < len; ++i) {
                            ...
                            _data.label = data.labels && data.labels[opt[i]] ? data.labels[opt[i]] : self.label['query_' + opt[i]];
						...
						if ( _data.query_filter ) {
							...
							if ( typeof _data.query_filter === 'object' ) {
								Object.keys( _data.query_filter ).forEach( function( i ) {
									let options = _data.query_filter[ i ].render(),
										has_value = _data.query_filter[ i ].validate( self.values );
									options.wrap_class = 'tb_query_filter' + ( has_value ? '' : ' tf_hide' );
									console.log(options);
									fr.appendChild( self.create( [ options ] ) );
									if ( ! has_value ) {
										hide_add_new = false;
										let li = document.createElement('li');
										li.innerText = _data.query_filter[ i ].label;
