...
final class Themify_Builder_Stylesheet {
	...
	*/
    private  static function reSaveData($data){
        foreach($data as &$r){
            ...
            $r['element_id'] = Themify_Builder_Model::checkUniqId((isset($r['element_id'])?$r['element_id']:null));
            unset($r['row_order']);
            if (!empty($r['cols'])) {
                foreach($r['cols'] as &$c){
                    $c['element_id'] = Themify_Builder_Model::checkUniqId((isset($c['element_id'])?$c['element_id']:null));
                    ...
                    if (!empty($c['modules'])) {
                        foreach($c['modules'] as &$m){
                            ...
                            $m['element_id'] = Themify_Builder_Model::checkUniqId((isset($m['element_id'])?$m['element_id']:null));
                            unset($m['row_order']);
                            ...
                            if (!empty($m['cols'])) {
                                foreach ($m['cols'] as &$sub_col) {
                                    $sub_col['element_id'] = Themify_Builder_Model::checkUniqId((isset($sub_col['element_id'])?$sub_col['element_id']:null));
                                    ...
                                    if (!empty($sub_col['modules'])) {
                                        foreach ($sub_col['modules'] as &$sub_m) {
                                            $sub_m['element_id'] = Themify_Builder_Model::checkUniqId((isset($sub_m['element_id'])?$sub_m['element_id']:null));
	...
	*/
	...
	public static function regenerate_css_files_ajax() {
	    ...
	    die(self::regenerate_css_files($type));
	}
			...
			if($write) {
				...
				if ( ! empty( $fonts ) ) {
					foreach ( array_keys( $fonts ) as $ftype ) {
						...
						}else if ( isset( $builder_fonts[ $style_id ] ) ) {
