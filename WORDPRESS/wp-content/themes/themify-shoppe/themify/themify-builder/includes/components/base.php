...
class Themify_Builder_Component_Base {
    ...
    public function get_settings() {
		return [
			[ 'id' => 'custom_css_' . $this->get_name(), 'type' => 'custom_css' ],
			[ 'type' => 'tooltip' ],
			...
			array(
				...
				'options' => array(
					...
					array(
						...
						'type' => 'range',
						'class' => 'tb_frame',
						'tooltip' => __( 'Blur', 'themify' ),
					),
					array(
						'id' => $id . '-frame_sh_c',
						'type' => 'color',
						'class' => 'tb_frame',
						'tooltip' => __( 'Color', 'themify' ),
					),
				)
			),
			array(
	...
	protected static function show_frame( $styles, $printed=array()) {
		...
		if ( ! empty( $output ) ) {
                    Themify_Builder_Model::loadCssModules('tb_fr', THEMIFY_BUILDER_CSS_MODULES . 'frames.css',THEMIFY_VERSION);
	...
	if (!empty($images)) :
	    ...
	    <div class="tf_hide <?php echo $type; ?>-slider tb_slider" data-bgmode="<?php echo $bgmode; ?>" data-sliderspeed="<?php echo $slider_speed ?>">
			...
			<li data-bg="<?php echo esc_url(themify_https_esc($img_data[0])); ?>" data-bg-alt="<?php echo esc_attr(get_post_meta($img->ID, '_wp_attachment_image_alt', TRUE)); ?>" class="normal">
			    <a class="row-slider-dot" data-index="<?php echo $i; ?>" href="javascript:void(0)" rel="nofollow" tabindex="0" role="button" aria-label="Go to slide"><span class="screen-reader-text">&bull;</span></a>
			</li>
			<?php
		    }
		    ?>
