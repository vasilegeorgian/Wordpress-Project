...
class TB_Feature_Module extends Themify_Builder_Component_Module {
	...
	var w=data.circle_stroke_feature || 0,
		...
		var layout_mobile = '';
		if ( data.layout_mobile ) {
			layout_mobile = ' data-layout-mobile="' + data.layout_mobile + '" data-layout-desktop="' + data.layout_feature + '"';
			if ( data.layout_mobile !== data.layout_feature && data.layout_mobile !== 'icon-top' ) {
				Themify.LoadCss( themifyBuilder.builder_url + '/css/modules/feature_styles/' + data.layout_mobile.replace( 'icon-', '' ) + '.css' );
			}
		}
        #>
        <div class="module module-<?php echo $this->slug; ?> {{chart_class}} layout-{{ data.layout_feature }} size-{{data.circle_size_feature}} {{ data.css_feature }}" {{{ layout_mobile }}}>
