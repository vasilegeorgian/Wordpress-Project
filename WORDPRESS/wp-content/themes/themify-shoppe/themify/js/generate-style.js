    ...
    ThemifyStyles = {
        ...
        fields: {
            ...
            padding(id, type, args, data) {
                ...
                //in fw v7 padding/margin of columns are saved as "value1,value2"(only when unit is %), where value1 belongs to v5 and value2 belongs to v7
                v=v.toString().trim().split(',');
                if(v[0]!==undefined && v[0]!==''){//value is exist in version v5
                    v=v[0];
                }
                else if(v[1]!==undefined && v[1]!==''){//value is exist only in version v7(created in v7),old version value we don't have
                    v=v[1];
                }
                else{
                    delete data[id + '_unit'];
        ...
        var Regenerate = function () {
            for (var k in window) {
                if (k.indexOf('themify_builder_data_') === 0 && window[k]!==null) {
                    ...
                    var css = ThemifyStyles.createCss(window[k]['data'], null, true,window[k]['gs']),
                    ...
                    cssFonts = null;
                    if(typeof window[k]['custom_css'] !== 'undefined'){
                        var cst = document.createElement('style');
                        cst.type = 'text/css';
                        cst.id = 'tb_temp_styles_custom_css' + id;
                        cst.appendChild( document.createTextNode( window[k]['custom_css'] ) );
                        d.appendChild(cst);
                    }
                    ...
                    body = '';
                    if(typeof window[k]['custom_css'] !== 'undefined'){
                        data['custom_css'] = window[k]['custom_css'];
                    }
                    for (var i in data) {
                        if (body !== '') {
                            body += '&';
